...
3. ~~**【F4 範圍】SRS 用 Leitner 盒還是完整 SM-2？**~~ ✅ **已解決（2026-05-19）**：owner 選 **Leitner 盒**，間隔預設 **1/3/7/16/35 天**（可調）。F0 已預留的 `due` 沿用；新增 `box` 欄位；`ease` 維持保留不啟用。Phase 2 推進方式亦拍板：**分段 F6 → F4 → F5 → Tier1，每段驗證**。
