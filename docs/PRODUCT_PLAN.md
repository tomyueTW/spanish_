...
**內容**：
1. ✅ **F8 結果頁美學收斂**（已完成，commit 見下）：保留 emoji 與西語短語，
   但 `.result-emoji` 64→32px、`.result-title` 26→19px 等尺寸/層級收斂，
   貼齊極簡定位（非移除）。錯題行動化已於 Phase 1 F3 完成。
2. ✅ **F9 自製題庫 grammar 誠實告知（版 a）**（已完成）：自製題庫卡
   meta 標「無文法題」；使用中為自製題庫時，文法 mode-btn 描述就地
   改為「自製題庫不含文法題（僅單字／聽力）」。**不**擴充 MD codec（§5 Q5＝否）。
3. **Tier 2 機會性重構**：CSS 外置、views 拆分（純整理，未做，有空才做、不排期）。
**狀態**：F8 / F9 已完成（純 CSS + 小幅 JS，1 檔 +12/−6）。Phase 1–3 的
功能性項目全數落地，僅餘 Tier 2 機會性整理（刻意不排期）。
...
4. ~~**【F8 美學】結果頁 emoji 鼓勵語要移除嗎？**~~ ✅ **已解決（2026-05-19）**：**保留** emoji 正回饋，但**改成與目前極簡定位相符的風格**（收斂尺寸/層級、與中性色票一致，不走 Duolingo 大色塊）。F8 範圍＝視覺收斂（非移除）＋錯題行動化。

5. ~~**【F9 方向】自製題庫支援手寫 grammar 題？**~~ ✅ **已解決（2026-05-19）**：**否**。F9 永遠只做「誠實告知」版（在自製題庫卡／灰掉的文法按鈕旁明寫「自製題庫僅含單字／聽力」），不擴充 MD codec 支援手寫文法。
